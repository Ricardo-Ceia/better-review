...
use crate::domain::diff::{FileDiff, ReviewCounts, count_review_statuses};
use crate::services::git::{GitService, PushFailure};
#[cfg(test)]
use crate::services::opencode::WhyRiskLevel;
#[cfg(test)]
use crate::services::opencode::WhyTarget;
#[cfg(test)]
use crate::services::opencode::why_target_for_file;
use crate::services::opencode::{OpencodeService, OpencodeSession, WhyAnswer};
#[cfg(test)]
...
use self::command_palette::{
    CommandPaletteAction, command_palette_filtered_items, execute_command_palette_action,
};
use self::command_palette::{
...
#[cfg(test)]
use self::explain_history::ExplainRunStatus;
use self::explain_history::{
    ExplainRun, cancel_current_explain, handle_explain_history_key, open_explain_history,
};
#[cfg(test)]
use self::explain_history::{clear_history_run, move_explain_history_cursor};
use self::explain_render::{draw_explain_history, draw_explain_menu};
#[cfg(test)]
use self::explain_render::{
    explain_context_source_line, explain_menu_lines, explain_panel_lines, explain_scope_preview,
    loading_thinking_label, render_why_answer_lines,
};
use self::explain_request::{handle_explain_result, request_explain, retry_current_explain};
use self::github_token::{
    draw_github_token_prompt, handle_github_token_prompt_key, open_github_token_prompt,
};
use self::keybinding_picker::{
    draw_keybinding_picker, handle_keybinding_picker_key, open_keybinding_picker,
};
#[cfg(test)]
use self::keybindings::{
    command_binding, is_valid_keybinding_char, keybinding_conflict, set_command_binding,
};
use self::keybindings::{key_for, key_matches, key_status_label};
#[cfg(test)]
use self::model_picker::{auto_model_label, model_picker_cursor};
use self::model_picker::{
    current_model_choice, draw_model_picker, draw_saved_model_picker, ensure_model_present,
    explicit_model_choice, handle_model_picker_key, handle_saved_model_picker_key,
    model_picker_status_message, open_model_picker, open_saved_model_picker, resolved_why_model,
    saved_model_label, saved_model_picker_cursor, sync_model_picker_cursors,
...
use self::review_display::{
    diff_change_bar, diff_change_bar_color, diff_change_bar_style, diff_content_style,
    diff_current_line_bg, diff_marker_style, diff_row_bg, line_number_style, review_marker,
    sidebar_file_label_parts, truncate_path,
};
use self::review_keys::handle_review_key;
use self::review_mutation::handle_hunk_sync_result;
use self::review_nav::{
...
use self::review_screen::draw_review;
#[cfg(test)]
use self::review_screen::{
    diff_scroll_offset, file_sidebar_icon, file_status_panel_label, render_review_unified_lines,
    spans_width,
};
        ...
        while let Ok(message) = app.rx.try_recv() {
            match message {
                ...
                Message::WhyThis {
                    ...
                    label,
                    result,
                } => handle_explain_result(&mut app, job_id, cache_key, label, result),
...
#[cfg(test)]
mod tests {
    use super::publish::publish_status_line;
    use super::*;
